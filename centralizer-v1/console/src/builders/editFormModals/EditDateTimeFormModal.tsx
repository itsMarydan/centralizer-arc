...
interface EditDateFormModalProps{
    fieldData: any,
    handleCloseEditDateTimeFormModal: any,
    showEditDateTimeFormModal: boolean,
    appSlug: string,
    slug: string,
    userRole: string,
    dataType: number
}
export function EditDateTimeFormModal( props: EditDateFormModalProps){
    const methods = useForm();
    const [loading, setLoading] = React.useState(false);
    const [errorMessage, setErrorMessage] = React.useState("");
    const dispatch = useAppDispatch();
    async function onSubmit(data: any){
        data.contentType = FieldTypeName.DATE_TIME;
        ...
        function onSuccess() {
            ...
            setLoading(false);
            props.handleCloseEditDateTimeFormModal();
        }
    ...
    return (
        <>
            <Modal show={props.showEditDateTimeFormModal} onHide={props.handleCloseEditDateTimeFormModal} size={"lg"} centered>
                    ...
                    <FormProvider {...methods}>
                        <form onSubmit={methods.handleSubmit(onSubmit)}>
                            <div className="form">
                                ...
                                <div className="mb-3">
                                    <CheckBoxFormEdit default={props.fieldData.autoGenerate} id={"autoGenerate"} name={"Auto Generate"} />
                                </div>
